...
## "Creating correlation plot for pyrolysis gas chromatography-mass spectrometry
## data analyzed by random forest classification method"

## Data consist of 150 samples of organic molecular mixtures analyzed by 
## pyrolysis gas chromatography-mass spectrometry (py-GC-MS).
## Each data set has the scan values in the rows and the mass to charge ratio (m/z) values
## in the columns.
## The data values are the raw intensities as measured by py-GC-MS.
## Each sample has either an abiotic (A) or biotic (B) origin. 
## Purpose is to build classification rules to predict whether the samples are biotic or
## abiotic.
## Purpose is also to determine the features, (m/z) and (scan) values, that discriminate
## between the
## biotic and the abiotic groups.
## The data are preprocessed before we apply random forest.
## In this file we create correlation plot regarding the importance variables
...
corr_matB = round(cor(allB2),2)
#Omit the columns with only zeros
corr_matA = round(cor(allA2[,c(-2,-21)]),2) 

corrplot(corr_matB, tl.col ="black")
corrplot(corr_matA,tl.col ="black")
