## "Monte Carlo simulations using random forest applied to pyrolysis gas chromatography-mass
## spectrometry data"
...
## Running Monte Carlo simulations using random forest.
...
####################################################################
###Monte Carlo (MC) simulations using random forest

set.seed(99)
outer_loop = rsmp("cv", folds = 10)

dff.list=list()  #List of importance variables from the MC simulations
Test.list=list() #List of test errors from the MC simulations
...
test_set=  setdiff(seq(1,150),train_set)
y_train=y[train_set]
y_test=y[test_set]
...
#Create a task 
task=as_task_classif(training_transformed[train_set,], target = "y", positive="B")
...
#Using stratified random sampling inside each fold for tuning the parameters
instance = tune(
  tuner = RS,
  task =  task,
  learner = graph_learner,
  resampling = outer_loop,
  measure = msr("classif.ce"),
  term_evals = 20
)


learner3 = lrn("classif.ranger", id = "rf",predict_type="prob",
importance = "impurity")
learner3$param_set$values$mtry =       as.numeric(instance$result_learner_param_vals[7])
learner3$param_set$values$num.trees  =   as.numeric(instance$result_learner_param_vals[8])
    
#Train the model
learner3$train(task)

#Prediction on the test set
prediction_rf = learner3$predict_newdata(newdata = training_transformed[test_set,])
measure = msr("classif.ce")
Test.list[[k]] = as.numeric(prediction_rf$score(measure))

#Importance variables
filter = flt("importance", learner = learner3)
filter$calculate(task)

dff.list[[k]]=as.data.table(filter)

}
df.list = do.call(cbind, dff.list)
df.test = do.call(cbind, Test.list)


mean(as.numeric(df.test))# average test error
sd(as.numeric(df.test))
df.list[[1]][1:10]

###########################################################################
#Determine the importance variables that have lowest average rank across the
#Monte Carlo samples and that are among the original importance variables
...
#Lowest ranked importance variables in the original sample that are also among 
