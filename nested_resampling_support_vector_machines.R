...
## Support vector machines (SVM) are used as the machine learning methods.
## We apply nested resampling to obtain the generalized prediction performance.
## The data are preprocessed before we use machine learning.
...
library(praznik)       # for JMI filter
...
graph1 = po("removeconstants") %>>%  learner

graph_learner_1 = as_learner(graph1)

#next tuning the hyper parameters
graph_learner_1$param_set$values$classif.svm.cost  = 
  to_tune(p_dbl(-2, 2, trafo = function(x) 10^x))
graph_learner_1$param_set$values$classif.svm.gamma=
  to_tune(p_dbl(-5, 5, trafo = function(x) 10^x))
graph_learner_1$param_set$values$classif.svm.kernel=
  to_tune(c("polynomial", "radial"))
graph_learner_1$param_set$values$classif.svm.degree=
  to_tune(1, 4)

graph_learner_1$id = "graph_learner_1"
...
##########################
#Nested resampling, SVM with AUC filter
...
learner2 = lrn("classif.svm", type = "C-classification",predict_type="prob")

graph2 = po("removeconstants") %>>% po("filter", flt("auc")) %>>% 
  po("scale") %>>% po("pca") %>>% learner2
#graph2$param_set$ids()

graph_learner_2 = as_learner(graph2)


#next tuning the hyper parameters
graph_learner_2$param_set$values$classif.svm.cost  = 
  to_tune(p_dbl(-2, 2, trafo = function(x) 10^x))
graph_learner_2$param_set$values$classif.svm.gamma=
  to_tune(p_dbl(-5, 5, trafo = function(x) 10^x))
graph_learner_2$param_set$values$classif.svm.kernel=
  to_tune(c("polynomial", "radial"))
graph_learner_2$param_set$values$classif.svm.degree=
  to_tune(1, 4)
graph_learner_2$param_set$values$auc.filter.frac=
  to_tune(p_dbl(0,1))
graph_learner_2$param_set$values$pca.rank.=
  to_tune(p_int(1, 100))

graph_learner_2$id = "graph2"
...
rr2 = resample(task = task, learner = at2, resampling = outer_loop, 
               store_models = TRUE)
#extract_inner_tuning_results(rr2)

#Unbiased prediction performance (testing error)
rr2$aggregate()

rr2$aggregate(msr("classif.auc"))

##########################
#Nested resampling, SVM with JMI filter
...
learner3 = lrn("classif.svm", type = "C-classification",predict_type="prob")

graph3 = po("removeconstants") %>>% po("filter", flt("jmi")) %>>%  
  po("scale") %>>% po("pca") %>>% learner3

#graph3$param_set$ids()

graph_learner_3 = as_learner(graph3)



#next tuning the hyper parameters
graph_learner_3$param_set$values$classif.svm.cost  = 
  to_tune(p_dbl(-2, 2, trafo = function(x) 10^x))
graph_learner_3$param_set$values$classif.svm.gamma=
  to_tune(p_dbl(-5, 5, trafo = function(x) 10^x))
graph_learner_3$param_set$values$classif.svm.kernel=
  to_tune(c("polynomial", "radial"))
graph_learner_3$param_set$values$classif.svm.degree=
  to_tune(1, 4)
graph_learner_3$param_set$values$jmi.filter.frac=
  to_tune(p_dbl(0,1))
graph_learner_3$param_set$values$pca.rank.= 
  to_tune(p_int(1, 100))


graph_learner_3$id = "graph3"


future::plan("multisession")

at3 = AutoTuner$new(
  learner = graph_learner_3,
  resampling = inner_loop,
  measure = msr("classif.ce"),
  terminator = trm("evals", n_evals = 20),
  tuner = tnr("random_search"),
...
rr3 = resample(task = task, learner = at3, resampling = outer_loop, 
               store_models = TRUE)
#extract_inner_tuning_results(rr3)

#Unbiased prediction performance (testing error)
rr3$aggregate()

rr3$aggregate(msr("classif.auc"))


##################################
##########################
#Nested resampling, SVM with ANOVA filter
...
learner4 = lrn("classif.svm", type = "C-classification",predict_type="prob")

graph4 = po("removeconstants") %>>% po("filter", flt("anova")) %>>% 
  po("scale") %>>% po("pca") %>>% learner4

#graph4$param_set$ids()

graph_learner_4 = as_learner(graph4)


#next tuning the hyper parameters
graph_learner_4$param_set$values$classif.svm.cost  = 
  to_tune(p_dbl(-2, 2, trafo = function(x) 10^x))
graph_learner_4$param_set$values$classif.svm.gamma=
  to_tune(p_dbl(-5, 5, trafo = function(x) 10^x))
graph_learner_4$param_set$values$classif.svm.kernel=
  to_tune(c("polynomial", "radial"))
graph_learner_4$param_set$values$classif.svm.degree=
  to_tune(1, 4)
graph_learner_4$param_set$values$anova.filter.frac=
  to_tune(p_dbl(0,1))
graph_learner_4$param_set$values$pca.rank.= 
  to_tune(p_int(1, 100))

graph_learner_4$id = "graph4"

future::plan("multisession")

at4 = AutoTuner$new(
  learner = graph_learner_4,
  resampling = inner_loop,
  measure = msr("classif.ce"),
  terminator = trm("evals", n_evals = 20),
  tuner = tnr("random_search"),
...
rr4 = resample(task = task, learner = at4, resampling = outer_loop,
               store_models = TRUE)
#extract_inner_tuning_results(rr4)

#Unbiased prediction performance (testing error)
rr4$aggregate()

rr4$aggregate(msr("classif.auc"))
